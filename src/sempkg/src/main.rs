...
fn run_query(
    ...
    if !docs_only {
        // package may be docs-only; a resolve failure here is not fatal.
        if let Ok(path) = resolve_codegraph_path(package, workspace) {
            match graph::query(&path, query, kind, fetch_k) {
    ...
    if !code_only {
        ...
        // package may be symbols-only; a resolve failure here is not fatal.
