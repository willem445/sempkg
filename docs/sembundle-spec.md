...
├── config.json         # Indexer configuration used during indexing (required)
├── graph/              # Graph store directory (required)
│   └── codegraph.db    # Schema-v4 SQLite graph (semgraph- or CodeGraph-built)
├── embeddings/         # Indexer auxiliary data (required)
│   └── ...             # Internal files (implementation-defined by producer)
...
| `codegraph_version` | string          | Yes      | Free-form producer/version tag for the graph index. Native-built bundles use `"sempkg-native/<version>"`; CodeGraph-built bundles use the CodeGraph version (e.g. `"0.9.7"`). Not validated or format-checked by consumers. |
...
Describes the source repository and the indexing context. This file is generated by the packer from the indexer's output and source control information.
...
## 6. `config.json` — Indexer Configuration

Indexer configuration used to produce the index. Included in the bundle to
communicate indexing parameters to consumers.

The native `semgraph` indexer takes no external configuration file, so it writes
an empty object (`{}`). Bundles produced by CodeGraph 0.9.7 embed that tool's
configuration verbatim. Either way, `config.json` must be a valid JSON object; at
minimum `{}`.
...
The `graph/` directory contains the graph store: a single SQLite database,
`graph/codegraph.db`, in **CodeGraph schema version 4** (`schema_versions.version
= 4`). Since issue #78 this database is produced by the native `semgraph`
indexer (built into `sembundle`/`sempkg`); prior releases produced it with the
`@colbymchenry/codegraph` 0.9.7 CLI. The two are **byte-compatible at the schema
level** — same tables, indexes, and FTS5 triggers — so a bundle built by either
producer is readable by the same reader, and older CodeGraph-built bundles keep
working unchanged.

Consumers load `graph/codegraph.db` via a schema-v4 reader (sempkg uses the
native `semgraph` reader; **no CodeGraph/Node install is required** to query a
bundle). A reader encountering a `schema_versions.version` it does not support
must fail with an actionable error rather than silently mis-reading. At least one
file must be present inside `graph/`; an empty directory is invalid.

---

## 8. `embeddings/` — Embedding Vectors

The `embeddings/` directory carries indexer-produced auxiliary data alongside the
graph. The internal format is **implementation-defined by the producer**: the
native `semgraph` indexer writes a `source-index.json` manifest recording the
indexed source roots; CodeGraph-built bundles carry that tool's embedding
vectors.

At least one file must be present inside `embeddings/`; an empty directory is
invalid.
...
| Version | Date       | Notes |
|---------|------------|-------|
| 1.2.0   | 2026-07-09 | Documentation-only clarification (no on-disk format change): `graph/codegraph.db` is now produced by the native `semgraph` indexer (issue #78 cutover), still schema v4 and byte-compatible with CodeGraph-0.9.7-built bundles. Reworded §6–§8 to be producer-neutral; clarified `codegraph_version` (§4.2) is a free-form producer tag (`sempkg-native/<version>`), not validated. |
